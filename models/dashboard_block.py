...
class DashboardBlock(models.Model):
    ...
    def get_dashboard_vals(self, action_id, start_date=None, end_date=None):
        ...
        for rec in blocks:
            # Skip if user not in group
            if rec.group_id and rec.group_id not in user.groups_id:
                continue

            # Validate and parse filter safely
            try:
                filter_list = literal_eval(rec.filter) if rec.filter else []
                if not isinstance(filter_list, list):
                    filter_list = []
            except Exception:
                filter_list = []

            # Remove create_date filters if exist
            filter_list = [f for f in filter_list if not (isinstance(f, tuple) and f[0] == 'create_date')]
            ...
            if rec.filter_bool:
                if start_date and start_date != 'null':
                    try:
                        ...
                        filter_list.append(('create_date', '>=', start_date_obj.strftime('%Y-%m-%d')))
                    except Exception:
                        pass
                if end_date and end_date != 'null':
                    try:
                        end_date_obj = datetime.strptime(end_date, '%Y-%m-%d')
                        filter_list.append(('create_date', '<=', end_date_obj.strftime('%Y-%m-%d')))
                    except Exception:
                        pass
            ...
            # Add time period filters safely
            if rec.time_period and rec.filter_bool:
                ...
                try:
                    ...
                    _logger.warning("Error applying time_period filter: %s", e)
            ...
            if rec.model_name:
                try:
                    # Safety: measured_field_id must exist
                    if not rec.measured_field_id:
                        _logger.warning("Missing measured_field_id for block %s", rec.id)
                        continue
                    ...
                    if rec.type == 'graph':
                        ...
                        # Ensure params is a tuple, list, or dict before execute
                        ...
                        if records:
                            ...
                            records[0]['value'] = total
                            vals.update(records[0])
                        else:
                            vals.update({'value': 0})

                except Exception as e:
                    _logger.error("Error processing block %s: %s", rec.id, e)
                    vals.update({'value': 0, 'error': str(e)})
